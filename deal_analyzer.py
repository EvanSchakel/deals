...
def parse_price(raw: str) -> Optional[float]:
    ...
    for token in raw.split():
        ...
        # Prevent DoS from extremely long strings passed to float()
        if len(digits) > 30:
            continue
        try:
            value = float(digits)
...
def _safe_float(value: str) -> float:
    """Safely convert string to float preventing long-string DoS."""
    if len(value) > 30:
        raise argparse.ArgumentTypeError(f"Price string too long (max 30 chars).")
    try:
        return float(value)
    except ValueError:
